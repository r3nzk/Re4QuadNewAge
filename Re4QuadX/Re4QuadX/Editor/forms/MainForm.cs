...
using Re4QuadX.Editor.Class.MyProperty;
using Re4QuadX.Editor.Class.MyProperty._EFF_Property;
...
using Re4QuadX.Editor.Controls;
...
namespace Re4QuadX
{
    public partial class MainForm : Form
    {
        ...
        {
            while (!IsRenderLoopPaused && glControl != null && !glControl.IsDisposed && glControl.IsIdle)
            {
                ...
                if (isDraggingGizmo){
                    MoveGizmo(currentMouseDelta);
                    currentMouseDelta = Vector2.Zero;
                }

                if (objectMove.ActiveDrag != MoveControlType.Null && objectMove.LatestMouseEvent != null){
                    objectControl.PerformMove(objectMove.ActiveDrag, objectMove.LatestMouseEvent);
                }
        ...
        private void UpdateCameraMovement(float deltaTime)
        {
            ...
            {
                if (isWDown) { camera.updateCameraToFront(deltaTime); cameraMoved = true; }
                if (isSDown) { camera.updateCameraToBack(deltaTime); cameraMoved = true; }
                if (isDDown) { camera.updateCameraToRight(deltaTime); cameraMoved = true; }
                if (isADown) { camera.updateCameraToLeft(deltaTime); cameraMoved = true; }
                if (isQDown) { camera.updateCameraToDown(deltaTime); cameraMoved = true; }
                if (isEDown) { camera.updateCameraToUp(deltaTime); cameraMoved = true; }
            }
        ...
        private void Render()
        {
            ...
            {
                NewAgeTheRender.TheRender.AllRender(ref camMtx, ref ProjMatrix, camera.Position, camera.SelectedObjPosY(), gizmo, camera, Globals.CurrentTool, Globals.CurrentGizmoSpace, true);
            }
            else
            {
                NewAgeTheRender.TheRender.AllRender(ref camMtx, ref ProjMatrix, camera.Position, camera.SelectedObjPosY(), gizmo, camera, Globals.CurrentTool, Globals.CurrentGizmoSpace);
            }
        }
        ...
        {
            return Matrix4.CreatePerspectiveFieldOfView(Globals.FOV * ((float)Math.PI / 180.0f), (float)glControl.Width / (float)glControl.Height, 1f, 1_000_000f);
        }
        ...
        private void MoveGizmo(Vector2 mouseDelta)
        {
        ...
        private void toolStripMenuItemShowLateralMenu_Click(object sender, EventArgs e)
        {
            if (!toolStripMenuItemShowLateralMenu.Checked) //hide
            {
                splitContainerMain.Panel1.Enabled = true;
                splitContainerMain.Panel1Collapsed = false;

                toolStripMenuItemShowLateralMenu.Checked = false;
            }
            else //show
            {
                ...
                toolStripMenuItemShowLateralMenu.Checked = true;
            }
            glControl.Invalidate();
        }

        private void toolStripMenuItemShowUtilityMenu_Click(object sender, EventArgs e)
        {
            if (!toolStripMenuItemShowUtilityMenu.Checked) //hide
            {
                splitContainerRight.Panel2Collapsed = false;
                toolStripMenuItemShowUtilityMenu.Checked = false;
            }
            else //show
            {
                splitContainerRight.Panel2Collapsed = false;
                toolStripMenuItemShowUtilityMenu.Checked = true;
            }
