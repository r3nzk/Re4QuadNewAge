...
using JADERLINK_MODEL_VIEWER.Editor.Nodes;
using ViewerBase;
    ...
    public class ObjModel3D
    {
        ...
        private void Load2007(ObjectModel2007 objmodel2007)
        {
            ...
            foreach (var sub in objmodel2007.List)
            {
                ...
                {
                    RE4_2007_MODEL_VIEWER.Editor.LoadPmdModel loadPmdModel = new RE4_2007_MODEL_VIEWER.Editor.LoadPmdModel(modelGroup, mng, tpng);
                ...
                else
                {
                    ...
                    Re4QuadX.Editor.Console.Warning($"Failed to find 3D model '{sub.PmdFile}' for 2007/xFile in directory '{BaseDirectory ?? "null"}'. Check the path or folder order.");
                }
            }
        ...
        private void LoadUHD(ObjectModelUhd objmodelUhd)
        {
            ...
            foreach (var sub in objmodelUhd.List)
            {
                ...
                if (File.Exists(modelFullPath))
                {
                    LoadUhdPs4Ns.Editor.LoadUhdBinModel loadUhdBinModel = new LoadUhdPs4Ns.Editor.LoadUhdBinModel(modelGroup, mng, IsPS4NS);
                ...
                {
                    //bin file not found
                    //Re4QuadX.Editor.Console.Warning($"Failed to find 3D model '{sub.UhdBinFile}' for UHD in directory '{BaseDirectory ?? "null"}'. Check the path or folder order.");
                }

                if (File.Exists(tplFullPath))
                {
                    LoadUhdPs4Ns.Editor.LoadUhdTpl loadUhdTpl = new LoadUhdPs4Ns.Editor.LoadUhdTpl(modelGroup, mng, IsPS4NS);
            ...
            {
                RE4_UHD_MODEL_VIEWER.Editor.LoadUhdPackCustomQuad loadUhdPack = new RE4_UHD_MODEL_VIEWER.Editor.LoadUhdPackCustomQuad(modelGroup, tpng);
            ...
            {
                RE4_PS4NS_MODEL_VIEWER.Editor.LoadPs4NsPackCustomQuad loadPack = new RE4_PS4NS_MODEL_VIEWER.Editor.LoadPs4NsPackCustomQuad(modelGroup, tpng);
                foreach (var item in packIds)
                {
                    var PackFilepath = Path.Combine(PackBaseDirectory, objmodelUhd.PackFolder, item + ".pack");
        ...
        private void LoadPS2(ObjectModelPs2 objmodelPs2)
        {
            ...
            foreach (var sub in objmodelPs2.List)
            {
                ...
                if (File.Exists(modelFullPath))
                {
                    RE4_PS2_MODEL_VIEWER.Editor.LoadPs2BinModel loadUhdBinModel = new RE4_PS2_MODEL_VIEWER.Editor.LoadPs2BinModel(modelGroup, mng);
                ...
                else
                {
                    ...
                    //Re4QuadX.Editor.Console.Warning($"Failed to find 3D model '{sub.Ps2BinFile}' for PS2 in directory '{BaseDirectory ?? "null"}'. Check the path or folder order.");
                }

                if (File.Exists(tplFullPath))
                {
                    RE4_PS2_MODEL_VIEWER.Editor.LoadPs2Tpl loadUhdTpl = new RE4_PS2_MODEL_VIEWER.Editor.LoadPs2Tpl(modelGroup, mng);
